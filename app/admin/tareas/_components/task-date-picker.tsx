...
import { parseTaskDueDate } from "../_lib/task-date";
...
export function TaskDatePicker({
  ...
  const selectedDate = React.useMemo(
    () => parseTaskDueDate(date) ?? undefined,
    [date],
  );
      ...
      <div className={cn("relative flex items-center w-full", className)}>
        ...
        {selectedDate && !disabled && (
          <button
            type="button"
            onClick={handleClear}
            className="absolute right-2 top-1/2 -translate-y-1/2 h-5 w-5 rounded-full flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted/80 transition-colors z-10 cursor-pointer"
