  ...
  try {
    ...
    try {
      pyResponse = await fetch(
        `${PYTHON_SERVICE_URL}/api/v1/information-requests/sync`,
        {
          method: "POST",
          headers: {
      ...
      console.error(
        "No se pudo conectar con el servicio Python de pedidos de información:",
        connErr,
      );
      return NextResponse.json(
        {
          error:
            "No se pudo conectar con el servicio de pedidos de información (" +
            PYTHON_SERVICE_URL +
    ...
    if (!pyResponse.ok) {
      ...
      return NextResponse.json(
        { error: `Error en servicio de pedidos de información: ${err}` },
    ...
    console.error("Error en proxy de sync information requests:", error);
