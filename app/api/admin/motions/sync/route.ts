...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      pyResponse = await fetch(`${PYTHON_SERVICE_URL}/api/v1/motions/sync`, {
        method: "POST",
        headers: {
      ...
      console.error(
        "No se pudo conectar con el servicio Python de mociones:",
        connErr,
      );
      return NextResponse.json(
        {
          error:
            "No se pudo conectar con el servicio de mociones (" +
            PYTHON_SERVICE_URL +
    ...
    if (!pyResponse.ok) {
      ...
      return NextResponse.json(
        { error: `Error en servicio de mociones: ${err}` },
    ...
    console.error("Error en proxy de sync motions:", error);
