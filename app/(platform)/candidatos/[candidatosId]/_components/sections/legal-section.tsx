...
export function LegalSection({ person, legislatorId }: LegalSectionProps) {
  ...
  return (
    <section
      id="sec-legal"
      className="py-8 border-b border-border/70 scroll-mt-24 transition-colors"
    >
      <header className="flex items-baseline justify-between gap-4 flex-wrap mb-5">
        ...
        <span className="text-[11px] font-mono uppercase tracking-wider text-muted-foreground">
          Fuente: Voto Informado (JNE) · Reportes de Prensa
        </span>
      </header>

      {/* Padrones oficiales: REINFO (minería) y RNAS (colegios profesionales / abogados) */}
      <div className="mb-6 rounded-2xl border border-border/70 overflow-hidden bg-card">
        ...
        <RegistrosOficiales
          is_incumbent={person.is_incumbent}
          reinfo_status={person.reinfo_status}
          rnas_sanctions={person.rnas_sanctions}
          profession={person.profession}
          legislatorId={legislatorId}
        />
      </div>

      {/* Lista de Antecedentes declarados o investigados */}
        ...
        {backgrounds.length === 0 ? (
          <div className="p-8 rounded-2xl border border-border/60 bg-muted/20 text-center flex flex-col items-center gap-2">
            ...
            <p className="text-sm font-bold text-foreground">
              Sin historial legal encontrado
            </p>
            <p className="text-xs text-muted-foreground max-w-md leading-relaxed">
              No se identificaron sentencias judiciales declaradas en su hoja de
              vida ante el JNE ni reportes de investigación periodística
              registrados.
            </p>
          </div>
        ) : (
                ...
                >
                  ...
                  {isOpen && (
                    <div className="px-4 pb-4 pt-1 border-t border-border/40 space-y-3.5 bg-muted/10">
                      ...
                      {/* Bloque de Sanción o Fallo si existe */}
                      {bg.sanction && (
                      ...
                      {/* Enlace a Fuente (Voto Informado / Portal de noticias) */}
